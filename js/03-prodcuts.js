...
const productCardsTemplate = products.map(el => createProductCardTemplate(el)).join('');

const productListEl = document.querySelector('.js-products');

productListEl.innerHTML = productCardsTemplate;
...
productListEl.addEventListener('click', event => {
  ...
  const productCardId = Number(productCard.dataset.id);

  const productInfo = products.find(el => el.id === productCardId);
